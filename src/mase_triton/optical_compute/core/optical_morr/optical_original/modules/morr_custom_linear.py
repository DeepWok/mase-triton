...
class AllPassMORRLinear(ONNBaseLayer):
    """
    ...
    def __init__(
        ...
        super(AllPassMORRLinear, self).__init__()
        ...
        # M * N/k MORR grid
        self.grid_dim_x = int(np.ceil(self.in_features / (self.miniblock)))
        self.grid_dim_y = out_features
    ...
    def _compute_single_pass(
        ...
        """Helper method to compute a single pass through the MORR."""
        ### x : [bs, N/k, k]
        ### weights: [M, N/k, k]
        ...
        # Input scaling/biasing if enabled
        if self.trainable_morr_scale:
            x = x * self.morr_scale
        ...
        # Apply MORR transmission function
        x = self.mrr_roundtrip_phase_to_tr(x)

        # Flatten output
        x = morr_output_scale.matmul(
            x
        )  # [1, 1, 1, N/k] x [bs, M, N/k, 1] = [bs, M, 1, 1]
        x = x.squeeze(-1).squeeze(-1)  # [bs, M]

        return x

    def propagate_morr(
        self, weight: Tensor, x: Tensor, morr_output_scale: Optional[Tensor] = None
    ) -> Tensor:
        """
        ...
        Noted Here, we use 4-pass matmals to preserve pre-trained weight losslessly.
        """
        ### x : [bs, q, k]
        ### weights: [p, q, k]
        ...
        # Split weights and inputs into positive and negative parts
        pos_weight = F.relu(weight)
        neg_weight = -F.relu(-weight)  # |W-|

        x = x.view(-1, self.grid_dim_x, self.miniblock)  # [bs, q, k]
        pos_x = F.relu(x)
        neg_x = -F.relu(-x)  # |X-|

        if self.enable_thermal_crosstalk and self.crosstalk_factor > 1:
            # weight = weight * self.crosstalk_factor
            pos_weight = pos_weight * self.crosstalk_factor
            neg_weight = neg_weight * self.crosstalk_factor

        # Compute the four passes
        result_pp = self._compute_single_pass(
            pos_weight, pos_x, morr_output_scale
        )  # 1. W+X+
        result_np = self._compute_single_pass(
            neg_weight, pos_x, morr_output_scale
        )  # 2. |W-|X+
        result_pn = self._compute_single_pass(
            pos_weight, neg_x, morr_output_scale
        )  # 3. W+|X-|
        result_nn = self._compute_single_pass(
            neg_weight, neg_x, morr_output_scale
        )  # 4. W-X-

        x = result_pp - result_np - result_pn + result_nn
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # ignore morr_output_scale, as we apply a uniform scale for all rows.

    ...
    def forward(ctx, x, quant_levels, min_val, max_val, quant_mode, lut_min=None):
        with torch.no_grad():
            ...
            if quant_mode == "det":
                ...
                x = x.add(
                    torch.rand_like(x).add(-0.5)
                ).round()  # randn* 0.288 gives same std as 0-1 rand(), if want to use normal dist.
                x = x.div(quant_levels - 1).mul(diff).add(min_val)
        ...
        if self.use_clipping:
            if not self.calibration_done.item():
                ...
                with torch.no_grad():
                    ...
                    if finished:
                        ...
                        self.running_max = torch.tensor(
                            maximum, device=tensor.device, dtype=tensor.dtype
                        )
                        minimum = tensor.min()
                        minimum = minimum if minimum >= 0.0 else -maximum
                        self.running_min = torch.tensor(
                            minimum, device=tensor.device, dtype=tensor.dtype
                        )
            ...
            with torch.no_grad():
                ...
                if self.initial_calibration_steps == 0:
                    ...
                    self.running_min = (
                        alpha * self.running_min + (1.0 - alpha) * cur_min
                    )
                    self.running_max = (
                        alpha * self.running_max + (1.0 - alpha) * cur_max
                    )
...
class MORRFF(nn.Module):
    def __init__(self, embed_dim, expansion_dim):
        ...
        self.layer1 = AllPassMORRCirculantLinear(
            embed_dim, expansion_dim, use_noise=True
        )
        self.act = nn.ReLU6(inplace=True)
        self.dropout = nn.Dropout(0.1)
        self.layer2 = AllPassMORRCirculantLinear(
            expansion_dim, embed_dim, use_noise=True
        )
...
class _MORRGPT(nn.Module):
    def __init__(self, features, heads, tokenizer, layers, max_length):
        ...
        vocab_size = (
            len(tokenizer) + 8 - len(tokenizer) % 8
        )  # pad vocab size to 8-multiple for tensor core acceleration
        assert vocab_size % 8 == 0
        self.pos_embedding = nn.Embedding(max_length, features)
        self.word_embedding = nn.Embedding(
            vocab_size, features, padding_idx=tokenizer.pad_token_id
        )
        self.embedding_dropout = nn.Dropout(0.1)
        self.decoders = nn.ModuleList(
            [MORRDecoderLayer(features, heads) for _ in range(layers)]
        )
...
class MORRGPT(pl.LightningModule):
    ...
    def configure_optimizers(self):
        if self.use_adam:
            ...
            scheduler = transformers.get_cosine_schedule_with_warmup(
                optimizer, num_warmup_steps=2500, num_training_steps=self.training_steps
            )
            return {
                "optimizer": optimizer,
                "lr_scheduler": {
    ...
    def enable_quantization(self):
        for m in self.transformer.modules():
            if isinstance(m, AllPassMORRCirculantLinear) or isinstance(
                m, AllPassMORRCirculantMatMals
            ):
